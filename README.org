...
- record [clojure defrecord] <-> goops object [scheme] (slots accessed by name)
- atom [clojure] <-> ice-9 atomic [scheme]

The operations are:
- get, get-in (vector, hash-table, goops object)
- assoc, assoc-in (vector, hash-table, goops object)
- update, update-in (vector, hash-table, goops object)
...
For goops objects slots are addressed by their symbol name: ~(get obj 'slot)~, ~(assoc obj 'slot val)~. As with the other structures, ~assoc~/~assoc-in~ are functional --- they return a ~shallow-clone~ with the slot updated and leave the original untouched.
** Installation
...
(define-public clojureism
 (let [(version "0.0.3")
  ...
  (package
   ...
   (native-inputs (list guile-3.0))
   (synopsis "Small guile scheme libriary to provide clojure-alike atom, vector, hash-map and goops-object basic procedures")
...
Structures correspondence: vector [clojure] <-> vector [scheme], hash-map [clojure] <-> srfi-125 hash-table [scheme], record [clojure] <-> goops object [scheme], atom [clojure] <-> ice-9 atomic [scheme].
Operations: get, get-in (vector, hash-table, goops object); assoc, assoc-in (vector, hash-table, goops object); update, update-in (vector, hash-table, goops object); ref, swap!, reset! (atomic).
