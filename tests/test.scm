(define-module (test)
 ...
 #:use-module (clojureism hash-table)
...
(define-test assoc-test
 (test-group "assoc/hash-table"
  (test-assert (hash-table-compare equal? (alist->hash-table `((a . 1) (b . 2))) (assoc table* 'a 1 'b 2))))
 ...
 (test-group "assoc-in/hash-table"
  ...
  (test-assert
   (hash-table-compare equal?
    (alist->hash-table
     `((hello . world)
       (a . ,(alist->hash-table `((b . ,(alist->hash-table `((c . 12)))))))
       (nested . ,(alist->hash-table
                   `((key1 . val1)
                     (key2 . ,(alist->hash-table
                               `((key1- . val1-)))))))))
